"""
identical_property_fields

Autogenerated DPF operator classes.
"""

from __future__ import annotations
from typing import TYPE_CHECKING

from warnings import warn
from ansys.dpf.core.dpf_operator import Operator
from ansys.dpf.core.inputs import Input, _Inputs
from ansys.dpf.core.outputs import Output, _Outputs
from ansys.dpf.core.operators.specification import PinSpecification, Specification
from ansys.dpf.core.config import Config
from ansys.dpf.core.server_types import AnyServerType

if TYPE_CHECKING:
    from ansys.dpf.core.meshed_region import MeshedRegion


class identical_property_fields(Operator):
    r"""Takes two property fields and compares them.


    Inputs
    ------
    order_independent: bool
        if true, the comparison between the data for each entity is order-independent (e.g. data 1,4,5 associated to id 7 is considered identical to data 4,5,1 associated to the same id). The default is false.
    property_fieldA: MeshedRegion
    property_fieldB: MeshedRegion

    Outputs
    -------
    are_identical: bool
    information: str

    Examples
    --------
    >>> from ansys.dpf import core as dpf

    >>> # Instantiate operator
    >>> op = dpf.operators.logic.identical_property_fields()

    >>> # Make input connections
    >>> my_order_independent = bool()
    >>> op.inputs.order_independent.connect(my_order_independent)
    >>> my_property_fieldA = dpf.MeshedRegion()
    >>> op.inputs.property_fieldA.connect(my_property_fieldA)
    >>> my_property_fieldB = dpf.MeshedRegion()
    >>> op.inputs.property_fieldB.connect(my_property_fieldB)

    >>> # Instantiate operator and connect inputs in one line
    >>> op = dpf.operators.logic.identical_property_fields(
    ...     order_independent=my_order_independent,
    ...     property_fieldA=my_property_fieldA,
    ...     property_fieldB=my_property_fieldB,
    ... )

    >>> # Get output data
    >>> result_are_identical = op.outputs.are_identical()
    >>> result_information = op.outputs.information()
    """

    def __init__(
        self,
        order_independent=None,
        property_fieldA=None,
        property_fieldB=None,
        config=None,
        server=None,
    ):
        super().__init__(
            name="compare::property_field",
            config=config,
            server=server,
            inputs_type=InputsIdenticalPropertyFields,
            outputs_type=OutputsIdenticalPropertyFields,
        )
        if order_independent is not None:
            self.inputs.order_independent.connect(order_independent)
        if property_fieldA is not None:
            self.inputs.property_fieldA.connect(property_fieldA)
        if property_fieldB is not None:
            self.inputs.property_fieldB.connect(property_fieldB)

    @staticmethod
    def _spec() -> Specification:
        description = r"""Takes two property fields and compares them.
"""
        spec = Specification(
            description=description,
            map_input_pin_spec={
                -1: PinSpecification(
                    name="order_independent",
                    type_names=["bool"],
                    optional=False,
                    document=r"""if true, the comparison between the data for each entity is order-independent (e.g. data 1,4,5 associated to id 7 is considered identical to data 4,5,1 associated to the same id). The default is false.""",
                ),
                0: PinSpecification(
                    name="property_fieldA",
                    type_names=["abstract_meshed_region"],
                    optional=False,
                    document=r"""""",
                ),
                1: PinSpecification(
                    name="property_fieldB",
                    type_names=["abstract_meshed_region"],
                    optional=False,
                    document=r"""""",
                ),
            },
            map_output_pin_spec={
                0: PinSpecification(
                    name="are_identical",
                    type_names=["bool"],
                    optional=False,
                    document=r"""""",
                ),
                1: PinSpecification(
                    name="information",
                    type_names=["string"],
                    optional=False,
                    document=r"""""",
                ),
            },
        )
        return spec

    @staticmethod
    def default_config(server: AnyServerType = None) -> Config:
        """Returns the default config of the operator.

        This config can then be changed to the user needs and be used to
        instantiate the operator. The Configuration allows to customize
        how the operation will be processed by the operator.

        Parameters
        ----------
        server:
            Server with channel connected to the remote or local instance. When
            ``None``, attempts to use the global server.

        Returns
        -------
        config:
            A new Config instance equivalent to the default config for this operator.
        """
        return Operator.default_config(name="compare::property_field", server=server)

    @property
    def inputs(self) -> InputsIdenticalPropertyFields:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsIdenticalPropertyFields.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsIdenticalPropertyFields:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsIdenticalPropertyFields.
        """
        return self._outputs


class InputsIdenticalPropertyFields(_Inputs):
    """Intermediate class used to connect user inputs to
    identical_property_fields operator.

    Examples
    --------
    >>> from ansys.dpf import core as dpf
    >>> op = dpf.operators.logic.identical_property_fields()
    >>> my_order_independent = bool()
    >>> op.inputs.order_independent.connect(my_order_independent)
    >>> my_property_fieldA = dpf.MeshedRegion()
    >>> op.inputs.property_fieldA.connect(my_property_fieldA)
    >>> my_property_fieldB = dpf.MeshedRegion()
    >>> op.inputs.property_fieldB.connect(my_property_fieldB)
    """

    def __init__(self, op: Operator):
        super().__init__(identical_property_fields._spec().inputs, op)
        self._order_independent: Input[bool] = Input(
            identical_property_fields._spec().input_pin(-1), -1, op, -1
        )
        self._inputs.append(self._order_independent)
        self._property_fieldA: Input[MeshedRegion] = Input(
            identical_property_fields._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._property_fieldA)
        self._property_fieldB: Input[MeshedRegion] = Input(
            identical_property_fields._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._property_fieldB)

    @property
    def order_independent(self) -> Input[bool]:
        r"""Allows to connect order_independent input to the operator.

        if true, the comparison between the data for each entity is order-independent (e.g. data 1,4,5 associated to id 7 is considered identical to data 4,5,1 associated to the same id). The default is false.

        Returns
        -------
        input:
            An Input instance for this pin.

        Examples
        --------
        >>> from ansys.dpf import core as dpf
        >>> op = dpf.operators.logic.identical_property_fields()
        >>> op.inputs.order_independent.connect(my_order_independent)
        >>> # or
        >>> op.inputs.order_independent(my_order_independent)
        """
        return self._order_independent

    @property
    def property_fieldA(self) -> Input[MeshedRegion]:
        r"""Allows to connect property_fieldA input to the operator.

        Returns
        -------
        input:
            An Input instance for this pin.

        Examples
        --------
        >>> from ansys.dpf import core as dpf
        >>> op = dpf.operators.logic.identical_property_fields()
        >>> op.inputs.property_fieldA.connect(my_property_fieldA)
        >>> # or
        >>> op.inputs.property_fieldA(my_property_fieldA)
        """
        return self._property_fieldA

    @property
    def property_fieldB(self) -> Input[MeshedRegion]:
        r"""Allows to connect property_fieldB input to the operator.

        Returns
        -------
        input:
            An Input instance for this pin.

        Examples
        --------
        >>> from ansys.dpf import core as dpf
        >>> op = dpf.operators.logic.identical_property_fields()
        >>> op.inputs.property_fieldB.connect(my_property_fieldB)
        >>> # or
        >>> op.inputs.property_fieldB(my_property_fieldB)
        """
        return self._property_fieldB


class OutputsIdenticalPropertyFields(_Outputs):
    """Intermediate class used to get outputs from
    identical_property_fields operator.

    Examples
    --------
    >>> from ansys.dpf import core as dpf
    >>> op = dpf.operators.logic.identical_property_fields()
    >>> # Connect inputs : op.inputs. ...
    >>> result_are_identical = op.outputs.are_identical()
    >>> result_information = op.outputs.information()
    """

    def __init__(self, op: Operator):
        super().__init__(identical_property_fields._spec().outputs, op)
        self._are_identical: Output[bool] = Output(
            identical_property_fields._spec().output_pin(0), 0, op
        )
        self._outputs.append(self._are_identical)
        self._information: Output[str] = Output(
            identical_property_fields._spec().output_pin(1), 1, op
        )
        self._outputs.append(self._information)

    @property
    def are_identical(self) -> Output[bool]:
        r"""Allows to get are_identical output of the operator

        Returns
        -------
        output:
            An Output instance for this pin.

        Examples
        --------
        >>> from ansys.dpf import core as dpf
        >>> op = dpf.operators.logic.identical_property_fields()
        >>> # Get the output from op.outputs. ...
        >>> result_are_identical = op.outputs.are_identical()
        """
        return self._are_identical

    @property
    def information(self) -> Output[str]:
        r"""Allows to get information output of the operator

        Returns
        -------
        output:
            An Output instance for this pin.

        Examples
        --------
        >>> from ansys.dpf import core as dpf
        >>> op = dpf.operators.logic.identical_property_fields()
        >>> # Get the output from op.outputs. ...
        >>> result_information = op.outputs.information()
        """
        return self._information
