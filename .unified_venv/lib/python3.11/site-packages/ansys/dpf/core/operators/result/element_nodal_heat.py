...
class element_nodal_heat(Operator):
    r"""Read/compute element nodal heat by calling the readers defined by the
    ...
    element_nodal_heat fields contain STATIC and DAMPING forces stored as
    components (when available). STATIC: component 0. DAMPING: component 1.
    ...
    >>> op = dpf.operators.result.element_nodal_heat()
    ...
    def __init__(
        ...
        super().__init__(
            ...
            config=config,
            server=server,
            inputs_type=InputsElementNodalHeat,
            outputs_type=OutputsElementNodalHeat,
        )
    ...
    def _spec() -> Specification:
        description = r"""Read/compute element nodal heat by calling the readers defined by the
datasources. - The ‘requested_location’ and ‘mesh_scoping’ inputs are
processed to see if they need scoping transposition or result averaging.
The resulting output fields have a ‘Nodal’, ‘ElementalNodal’ or
‘Elemental’ location. - Once the need for averaging has been detected,
...
"""
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="ENF_Heat", server=server)

    @property
    def inputs(self) -> InputsElementNodalHeat:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsElementNodalHeat.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsElementNodalHeat:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsElementNodalHeat.
        """
        return self._outputs


class InputsElementNodalHeat(_Inputs):
    """Intermediate class used to connect user inputs to
    element_nodal_heat operator.
...
class OutputsElementNodalHeat(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(element_nodal_heat._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            element_nodal_heat._spec().output_pin(0), 0, op
        )
