...
"""
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="ECT_CNOS", server=server)

    @property
    def inputs(self) -> InputsNumSurfaceStatusChanges:
...
class InputsNumSurfaceStatusChanges(_Inputs):
    ...
    num_surface_status_changes operator.
    ...
    def __init__(self, op: Operator):
        ...
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            num_surface_status_changes._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._mesh_scoping: Input[ScopingsContainer | Scoping] = Input(
            num_surface_status_changes._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._mesh_scoping)
        self._fields_container: Input[FieldsContainer] = Input(
            num_surface_status_changes._spec().input_pin(2), 2, op, -1
        )
        self._inputs.append(self._fields_container)
        self._streams_container: Input[StreamsContainer] = Input(
            num_surface_status_changes._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            num_surface_status_changes._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._mesh: Input[MeshedRegion | MeshesContainer] = Input(
            num_surface_status_changes._spec().input_pin(7), 7, op, -1
        )
        self._inputs.append(self._mesh)
        self._requested_location: Input[str] = Input(
            num_surface_status_changes._spec().input_pin(9), 9, op, -1
        )
        ...
        self._phi: Input[float] = Input(
            num_surface_status_changes._spec().input_pin(19), 19, op, -1
        )
        ...
        self._split_shells: Input[bool] = Input(
            num_surface_status_changes._spec().input_pin(26), 26, op, -1
        )
...
class OutputsNumSurfaceStatusChanges(_Outputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._fields_container: Output[FieldsContainer] = Output(
            num_surface_status_changes._spec().output_pin(0), 0, op
        )
