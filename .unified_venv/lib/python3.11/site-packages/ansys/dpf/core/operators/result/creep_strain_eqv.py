...
"""
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="EPCR_EQV", server=server)

    @property
    def inputs(self) -> InputsCreepStrainEqv:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsCreepStrainEqv.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsCreepStrainEqv:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsCreepStrainEqv.
        """
        return self._outputs


class InputsCreepStrainEqv(_Inputs):
    """Intermediate class used to connect user inputs to
    creep_strain_eqv operator.
...
class OutputsCreepStrainEqv(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(creep_strain_eqv._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            creep_strain_eqv._spec().output_pin(0), 0, op
        )
