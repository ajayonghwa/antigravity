...
class flow_rate(Operator):
    r"""Read/compute flow rate by calling the readers defined by the
    datasources.
    ...
    >>> op = dpf.operators.result.flow_rate()
    ...
    def __init__(
        ...
        super().__init__(
            ...
            config=config,
            server=server,
            inputs_type=InputsFlowRate,
            outputs_type=OutputsFlowRate,
        )
    ...
    def _spec() -> Specification:
        description = r"""Read/compute flow rate by calling the readers defined by the
datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="FlowRate", server=server)

    @property
    def inputs(self) -> InputsFlowRate:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsFlowRate.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsFlowRate:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsFlowRate.
        """
        return self._outputs


class InputsFlowRate(_Inputs):
...
class OutputsFlowRate(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(flow_rate._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            flow_rate._spec().output_pin(0), 0, op
        )
