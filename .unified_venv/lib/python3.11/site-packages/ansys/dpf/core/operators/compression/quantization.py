...
class InputsQuantization(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._input_field: Input[Field] = Input(
            quantization._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._input_field)
        self._threshold: Input[float | Field] = Input(
            quantization._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._threshold)

    @property
    def input_field(self) -> Input[Field]:
...
class OutputsQuantization(_Outputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._output_field: Output[Field] = Output(
            quantization._spec().output_pin(0), 0, op
        )
