...
class InputsOnProperty(_Inputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._requested_location: Input[str] = Input(
            on_property._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._requested_location)
        self._property_name: Input[str] = Input(
            on_property._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._property_name)
        self._property_id: Input[int] = Input(
            on_property._spec().input_pin(2), 2, op, -1
        )
        self._inputs.append(self._property_id)
        self._streams_container: Input[StreamsContainer] = Input(
            on_property._spec().input_pin(3), 3, op, -1
        )
    ...
    @property
    def property_id(self) -> Input[int]:
...
class OutputsOnProperty(_Outputs):
    ...
    def __init__(self, op: Operator):
        ...
        self._mesh_scoping: Output[Scoping] = Output(
            on_property._spec().output_pin(0), 0, op
        )
