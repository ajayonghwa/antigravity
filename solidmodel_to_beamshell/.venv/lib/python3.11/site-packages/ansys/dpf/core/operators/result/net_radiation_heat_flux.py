...
class net_radiation_heat_flux(Operator):
    r"""Read/compute net radiation heat flux by calling the readers defined by
    the datasources.
    ...
    >>> op = dpf.operators.result.net_radiation_heat_flux()
    ...
    def __init__(
        ...
        super().__init__(
            name="NetRadiationHeatFlux",
            config=config,
            server=server,
            inputs_type=InputsNetRadiationHeatFlux,
            outputs_type=OutputsNetRadiationHeatFlux,
        )
    ...
    def _spec() -> Specification:
        description = r"""Read/compute net radiation heat flux by calling the readers defined by
the datasources.
"""
        spec = Specification(
    ...
    def default_config(server: AnyServerType = None) -> Config:
        ...
        """
        return Operator.default_config(name="NetRadiationHeatFlux", server=server)

    @property
    def inputs(self) -> InputsNetRadiationHeatFlux:
        """Enables to connect inputs to the operator

        Returns
        --------
        inputs:
            An instance of InputsNetRadiationHeatFlux.
        """
        return self._inputs

    @property
    def outputs(self) -> OutputsNetRadiationHeatFlux:
        """Enables to get outputs of the operator by evaluating it

        Returns
        --------
        outputs:
            An instance of OutputsNetRadiationHeatFlux.
        """
        return self._outputs


class InputsNetRadiationHeatFlux(_Inputs):
    """Intermediate class used to connect user inputs to
    net_radiation_heat_flux operator.
    ...
    def __init__(self, op: Operator):
        super().__init__(net_radiation_heat_flux._spec().inputs, op)
        self._time_scoping: Input[Scoping | int | float | Field] = Input(
            net_radiation_heat_flux._spec().input_pin(0), 0, op, -1
        )
        self._inputs.append(self._time_scoping)
        self._mesh_scoping: Input[ScopingsContainer | Scoping] = Input(
            net_radiation_heat_flux._spec().input_pin(1), 1, op, -1
        )
        self._inputs.append(self._mesh_scoping)
        self._fields_container: Input[FieldsContainer] = Input(
            net_radiation_heat_flux._spec().input_pin(2), 2, op, -1
        )
        self._inputs.append(self._fields_container)
        self._streams_container: Input[StreamsContainer] = Input(
            net_radiation_heat_flux._spec().input_pin(3), 3, op, -1
        )
        self._inputs.append(self._streams_container)
        self._data_sources: Input[DataSources] = Input(
            net_radiation_heat_flux._spec().input_pin(4), 4, op, -1
        )
        self._inputs.append(self._data_sources)
        self._mesh: Input[MeshedRegion | MeshesContainer] = Input(
            net_radiation_heat_flux._spec().input_pin(7), 7, op, -1
        )
...
class OutputsNetRadiationHeatFlux(_Outputs):
    ...
    def __init__(self, op: Operator):
        super().__init__(net_radiation_heat_flux._spec().outputs, op)
        self._fields_container: Output[FieldsContainer] = Output(
            net_radiation_heat_flux._spec().output_pin(0), 0, op
        )
